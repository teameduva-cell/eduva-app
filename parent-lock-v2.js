...
(function () {
    ...
    function lockIndicator() {
        ...
        if (on && !b) {
            ...
            if (tabs && tabs.parentElement) {
                ...
                b.id = 'pl-lock-ind';
                b.style.cssText = 'font-size:9px;font-weight:900;background:#eef2ff;color:#4338ca;padding:3px 8px;border-radius:99px;white-space:nowrap;';
                b.textContent = '🔒 Night Lock';
    ...
    function renderDashboard() {
        ...
        var html = '<div style="background:linear-gradient(135deg,#065f46,#0d9488);color:#fff;padding:14px 16px;display:flex;align-items:center;gap:10px;">'
            ...
            + '</div>'
            + '<button onclick="window.__plReset()" style="width:100%;padding:11px;background:#fee2e2;border:1px solid #fca5a5;border-radius:12px;color:#b91c1c;font-weight:900;font-size:12px;cursor:pointer;margin-top:4px;">🔄 सब Reset — सारे controls OFF</button>'
    ...
    window.__plReset = function () {
        if (!confirm('सारे Parent controls OFF ho jayenge. Pakka?')) return;
        var cfg = getCfg();
        cfg.nightLock = { on: false, mode: 'study', from: '22:00', to: '06:00', examRelax: true };
        cfg.communityOff = false; cfg.gamesAfterHw = false; cfg.dailyAlert = false;
        setCfg(cfg);
    ...
    function init() {
        ...
        try { applySideEffects(); } catch(e) {}
        try { checkFullLock(); } catch(e) {}
        setInterval(function(){ try { checkFullLock(); } catch(e){} }, 30000);
        setInterval(function(){ try { dailyAlertCheck(); } catch(e){} }, 60000);
        setTimeout(function(){ try { injectBtn(); } catch(e){} }, 1500);
    }
